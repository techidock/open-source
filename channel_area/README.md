...
Selecting the calculator icon switches to the Channel Calculations page. This view hides the canvas and shows the **Channel Properties** panel fixed on the right side so you can review dimensions while working through calculations. The page includes a **Water Level to Area** table where you choose the number of levels to compute; the first row assumes zero water above the silt and the last row represents the channel's maximum depth. A **Copy Table** button copies the table as tab-separated text so you can paste it straight into a spreadsheet.
