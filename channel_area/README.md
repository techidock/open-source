...
This is a simple Flask-based web application for calculating the cross-sectional area of water channels. Shapes are drawn as polygons that you can freely edit. The interface uses **Bootstrap-Flask** so it adapts well to mobile screens.
...
The **Display Settings** panel includes controls for measurements. **Show Measurements** toggles the visibility of all arrows and their table while **Enable Measurements** simply turns drawing mode on or off&mdash;existing arrows remain intact when the option is unchecked. Click once on the canvas to start an arrow, move the mouse to see the length and index number, then click again to fix it in place. The table lists each arrow with its distance, start and end coordinates and the vertex indexes if snapped. Every row also provides a colour picker and a **Remove** button that asks for confirmation before deleting the row. Arrowheads turn black while being dragged and follow the cursor until released. Use **Clear All** to delete every measurement; a confirmation prompt prevents accidental clearing. Distances show three decimals in metres, one in centimetres and none in millimetres.
