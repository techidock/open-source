...
The **Display Settings** panel includes an **Enable Measurements** option. Click once on the canvas to start an arrow, move the mouse to see the length, then click again to fix it in place. You may repeat this to create multiple arrows. Each arrow is listed in a table under *Display Properties* showing the distance and the start and end coordinates. If an arrowhead snaps to a vector point the corresponding vertex indexes appear in the table. Arrowheads turn black while being dragged and follow the cursor until released. Use **Clear** to remove all arrows. Distances show three decimals in metres, one in centimetres and none in millimetres.
