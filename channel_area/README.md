...
The page shows light grey borders along the left, right and bottom edges. The left bar and a matching right bar are split into numbered tool sections. The bottom bar is divided into ten segments. Its final segment contains three smaller cells: the Properties button on the left, a Full Screen shortcut beside it and an empty spot on the right. The zoom slider remains in the centre section. The first cell displays the currently selected tool. Any choice from the menus&mdash;including **Move Image** and **Measure**&mdash;updates this label. When the Edit tool is active a **Shape Scale** slider appears in the next cell so you can quickly resize the polygon.
