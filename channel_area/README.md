...
Selecting the calculator icon switches to the Channel Calculations page. This view hides the canvas and displays four panels arranged in a 2×2 grid (sections 0–3). The layout collapses to a single column on narrow screens. Section 0 contains the water and silt height inputs along with units and velocity. Section 1 lists the channel properties in a table and shows a small preview canvas of the channel design beneath it. Section 2 holds a “Lookup Table” showing water level versus area with units in the column headers, and section 3 shows a Matplotlib plot of the same data. Matplotlib runs with the "Agg" backend so the chart renders in environments without a display server. A **Copy Table** button beside the level count copies the lookup table as tab‑separated text so you can paste it directly into a spreadsheet. The flow rate in cubic metres per second updates automatically from the current water area and velocity.
...
A dashed orange rectangle indicates the shape's bounding box. Labels beside the rectangle show the current width and height in the chosen units and the same numbers appear in the Channel Properties panel on the calculations page. The **Display** tab lets you toggle **Show Vectors**, **Show Bounding Box** and **Show Measurements**. Measurement mode is activated from the tape‑measure icon in the **Canvas Tools** panel so only one edit tool is active at a time. The panel lists the maximum width and height along with the **Total Area**, **Water Area** and **Silt Area**.
...
The axes rest slightly inside the canvas so the origin is about 20&nbsp;px from the top and sides and 40&nbsp;px from the bottom. This keeps the x‑axis visible above the bottom bar while still filling the drawing area.
 The **Display** tab controls the visibility of measurements. **Show Measurements** toggles all arrows and their table while the tape-measure tool starts drawing mode. Click once on the canvas to begin an arrow, move the mouse to see the distance and index numbers, then click again to fix it in place. The table lists each arrow with its distance, start and end coordinates and the vertex indexes if snapped. Every row provides a colour picker and a **Remove** button that confirms before deleting the row. Arrowheads turn black while being dragged and follow the cursor until released. A **Clear All** button in the Measurements tab removes every arrow after confirmation. Distances show three decimals in metres, one in centimetres and none in millimetres. Measurements are stored in millimetres alongside the channel points so resizing the canvas or switching units doesn't alter their real-world length.
