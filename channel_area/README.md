...
The **Display Settings** panel includes controls for measurements. **Show Measurements** toggles the visibility of all arrows and their table. **Enable Measurements** lets you draw up to ten arrows. Click once on the canvas to start an arrow, move the mouse to see the length and index number, then click again to fix it in place. The table lists each arrow with its distance, start and end coordinates and the vertex indexes if snapped. Every row also provides a colour picker and a **Remove** button. Arrowheads turn black while being dragged and follow the cursor until released. Use **Clear All** to delete every measurement. Distances show three decimals in metres, one in centimetres and none in millimetres.
