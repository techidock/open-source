...
Section 6 hosts a canvas icon that opens the **Canvas Tools** panel. This floating menu matches the left bar width. Item 0 provides a canvas colour picker, item 1 activates the tape‑measure tool and item 2 offers a **Copy to clipboard** button.
...
section (0) opens the **Projects** menu containing **New Channel** and **Save Shape** options. Section 4 shows a pencil icon for the
...
The Edit Tools menu slides 10 px to the right when opened. Position 0 offers **Select/Move** with a move cursor. Position 1 enables **Add Points** with an icon shaped like a green cross on a black circle the same size as the vector markers, and position 2 activates **Delete Point**. Double‑click a marker in delete mode to confirm via a small dialog near the cursor. The image panel provides an **Upload** button and a **Move Image** toggle. When this tool is active a slider titled **Image Scale** appears in section 2 of the bottom bar so you can resize the picture. The tape‑measure tool now resides in the **Canvas Tools** panel at position 4. The Canvas Tools panel contains five numbered cells so more tools can be added later.
...
The page shows light grey borders along the left, right and bottom edges. The left bar and a matching right bar are split into numbered tool sections. The bottom bar is divided into ten segments. Its final segment contains three smaller cells: the Properties button on the left, a Full Screen shortcut beside it and an empty spot on the right. The centre section holds a zoom slider flanked by round minus and plus buttons coloured the same as the bar. The first cell displays the currently selected tool. Any choice from the menus&mdash;including **Move Image** and **Measure**&mdash;updates this label. When the Edit tool is active a **Shape Scale** slider appears in the next cell so you can quickly resize the polygon.
