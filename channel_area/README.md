...
The axes rest slightly inside the canvas so the origin is about 20&nbsp;px from all sides. This keeps the x‑axis visible above the bottom bar while still filling the drawing area.
...
Resizing the browser window keeps the same drawing scale. The canvas width and height values update automatically so the channel remains the same size on screen. A 20 px margin surrounds the canvas so points near the borders are easier to edit.
