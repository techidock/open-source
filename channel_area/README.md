...
The Edit Tools menu slides 10 px to the right when opened. Position 0 offers **Select/Move** with a move cursor, position 1 enables **Add Points** using a large green cross cursor, and position 2 activates **Delete Point**. Clicking a marker in delete mode shows a small confirmation box near the cursor. The image panel provides an **Upload** button and a **Move Image** toggle. When this tool is active a slider titled **Image Scale** appears in section 2 of the bottom bar so you can resize the picture. The tape‑measure tool now resides in the **Canvas Tools** panel at position 4.
...
The page shows light grey borders along the left, right and bottom edges. The left bar and a matching right bar are split into numbered tool sections. The bottom bar is divided into ten segments. Its final segment contains three smaller cells: the Properties button on the left, a Full Screen shortcut beside it and an empty spot on the right. The zoom slider remains in the centre section. The first cell displays the currently selected tool such as **Select/Move**, **Add** or **Delete**.
